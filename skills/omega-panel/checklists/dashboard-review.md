...
- [ ] Overview exceptions and next actions outrank healthy/zero summaries; equal card weight or forced equal heights do not create duplicate signals or large dead areas.
- [ ] Every metric supports a decision and has an owner/definition.
- [ ] Secondary detail uses progressive disclosure.
- [ ] Surfaces are not recursively boxed: spacing, alignment, headings, separators, or restrained muted/slate backgrounds provide ordinary grouping; every remaining nested border has a real containment, interaction, or independent-state reason.
...
- [ ] First-use, filtered-empty, permission-limited, load-failure, and true-zero states have cause-specific copy and recovery; a failed or restricted request is never presented as “No results.”
...
- [ ] Normal interactive hit areas are at least 44 by 44 CSS pixels; any dense exception meets the WCAG 2.2 AA 24-pixel/spacing rule and is documented.
- [ ] Normal text meets 4.5:1 contrast, essential controls/graphics meet 3:1, and operational copy is not made unreadable through sub-14-pixel type or over-muted dark-theme colors.

## Admin editors, settings, and accounts

- [ ] A large, multi-section, rich-content, or high-consequence form uses a standalone route with a discoverable command area; the primary Save/Publish/Create action remains reachable during normal scrolling and on mobile.
- [ ] The mutation lifecycle defines dirty, saving, saved, validation failure, server failure, offline, and conflict states plus unsaved route/tab/content-locale/browser-exit behavior.
- [ ] Actions belong to the surface they mutate, one primary action leads each scope, redundant edit-mode actions are absent, and every visible action is valid for the current record state and permission.
- [ ] Form sections use headings, spacing, separators, or restrained backgrounds before nested cards; short controls use an appropriate readable measure rather than filling a wide desktop column.
- [ ] Read-only facts use labeled text/metadata rather than editable-looking or clearable disabled inputs.
- [ ] User/account state is explicit (for example invited, active, locked, disabled); invitation, unlock, disable, role, and danger actions cannot contradict that state, and last-owner/access invariants are enforced server-side.
- [ ] Content editors expose draft/review/scheduled/published behavior, useful fallback object identity, preview/publish/schedule actions, authoritative scheduling time zone, revision conflict behavior, and media failure/alt-text handling when relevant.
- [ ] UI locale and content locale are separate controls with independent state, per-locale completeness/fallback/publish semantics, and unsaved-work protection.
- [ ] Tabs/local section navigation have semantic active/focus states, 44-pixel targets, keyboard behavior, deliberate zoom/mobile overflow, and no dirty-field loss when switching.
- [ ] Repeaters expose identity, count/limits, Add, remove/undo or confirmation, reorder with keyboard equivalence, per-item errors, stable keys, and no unlabeled-ellipsis-only action discovery.
- [ ] Helper and empty-state copy is task-oriented and free of framework names, schema/code constraints, and other implementation commentary.
