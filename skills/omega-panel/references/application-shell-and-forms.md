...
Keep global search and account/theme/locale actions in the application header. Keep page-specific search, filters, export, creation, range, and view actions inside the table/card/form/chart region they affect; never float table actions in a decorative page-introduction row. Preserve a card header's useful local title and concise description as a separate region from its toolbar. A visible page introduction is allowed only when it supplies necessary task instructions, onboarding, or object identity that cannot be carried by the breadcrumb and primary surface; record that exception rather than adding filler copy. An object editor may show the record's human-readable identity, but it must not recreate the generic hero pattern or detach editor commands from the editor. Do not add stat cards merely to fill space; omit them when they do not change a decision.

For standalone editors, publishing workflows, account/user administration, settings, tabs, and repeatable field groups, follow [admin-editors-and-settings.md](admin-editors-and-settings.md). Those pages must define command ownership, save boundaries, dirty/pending/conflict behavior, unsaved-navigation protection, read-only presentation, state-valid actions, and surface hierarchy rather than treating a long list of fields or nested cards as a complete workflow.
...
On narrow screens, replace the persistent sidebar/rail with a labeled navigation trigger and modal drawer or sheet. Reuse the application's shared drawer/sheet primitive when one exists so navigation inherits the same overlay lifecycle, focus containment, scroll locking, dismissal paths, reduced-motion handling, and exit presence as every other drawer; do not build a parallel fixed-aside/backdrop implementation in the shell. The navigation drawer slides from logical inline-start over about 250 ms with the same linear timing on entry and exit, while its backdrop fades in and remains steady until slide-out completes. Keep it mounted but inert through exit, then restore focus to the mobile trigger. Close through that lifecycle on the close button, backdrop, `Escape`, and destination selection. Give mobile navigation a dedicated compact width near the expanded desktop sidebar—typically about 18 rem—with a maximum such as `calc(100vw - 1rem)` so a visible backdrop gutter remains even on very narrow screens. It must not default to full viewport width.

At the mobile breakpoint, render or display only the navigation-drawer trigger in the application header. The desktop sidebar collapse/expand control belongs exclusively to the persistent desktop rail and must not appear beside the mobile trigger or inside the mobile drawer. Conversely, suppress the mobile drawer trigger at desktop sizes. The drawer renders the same permission-filtered navigation tree and account actions as the desktop shell without inheriting or mutating the persisted desktop collapsed preference. Do not use desktop hover-preview behavior on touch-only layouts.
...
- A content/action drawer slides along the inline axis from logical inline-end on entry and returns to inline-end on exit. A mobile navigation drawer is the deliberate exception and slides from logical inline-start. Animate only the panel's transform and use the same `linear` timing function in both directions; do not fade the drawer panel. Both placements mirror in RTL without changing DOM or keyboard order, and should be variants of the same shared drawer primitive rather than separate components.
...
- Keep desktop drawers compact: use approximately 24 rem for a small drawer, 28 rem by default, and 32 rem for a larger bounded form. Content/form drawers may use full viewport width on very narrow screens when their controls require it, but the mobile navigation drawer remains a compact sidebar-width panel with a viewport-safe gutter. Do not let routine filters or small forms consume an oversized share of a desktop viewport.
...
Validation is only one part of the mutation lifecycle. Long forms and editors also need a discoverable primary command, explicit dirty/saving/saved/server-error/conflict state, and protection against losing unsaved work. Do not hide those responsibilities inside field components or rely on a transient toast as the only save result. See [admin-editors-and-settings.md](admin-editors-and-settings.md).
